...
func inputStreamNull() async throws {
    let null = try await JSON.Value.decode(from: ByteArrayInputStream("null"))
    #expect(null == .null)
}

@Test("InputStream bool")
func inputStreamBool() async throws {
    let jsonTrue = try await JSON.Value.decode(from: ByteArrayInputStream("true"))
    #expect(jsonTrue == .bool(true))

    let jsonFalse = try await JSON.Value.decode(from: ByteArrayInputStream("false"))
    #expect(jsonFalse == .bool(false))
}

@Test("InputStream number")
func inputStreamNumber() async throws {
    let uint = try await JSON.Value.decode(from: ByteArrayInputStream("42"))
    #expect(uint == .number(.uint(42)))

    let int = try await JSON.Value.decode(from: ByteArrayInputStream("-42"))
    #expect(int == .number(.int(-42)))

    let double = try await JSON.Value.decode(from: ByteArrayInputStream("-42.42"))
    #expect(double == .number(.double(-42.42)))

    let dble = try await JSON.Value.decode(from: ByteArrayInputStream("-.42"))
    #expect(dble == .number(.double(-0.42)))

    let exp = try await JSON.Value.decode(from: ByteArrayInputStream("4.2e1"))
    #expect(exp == .number(.double(42)))

    await #expect(throws: JSON.Error.invalidJSON) {
        try await JSON.Value.decode(from: ByteArrayInputStream("-\(UInt.max)"))
    }

    await #expect(throws: Never.self) {
        try await JSON.Value.decode(from: ByteArrayInputStream("-\(Int.max)"))
    }
}

@Test("InputStream string")
func inputStreamString() async throws {
    let json = ByteArrayInputStream("\"string\"")
...
func inputStreamEsapedString() async throws {
    let escapedJson = ByteArrayInputStream("\"string\\r\\n\"")
...
func inputStreamInvalidEscapedString() async throws {
    let invalidJson = ByteArrayInputStream("\"string\r\n\"")
    await #expect(throws: JSON.Error.invalidJSON) {
...
func inputStreamEscapedUnicodeString() async throws {
    let escapedUnicodeJson = ByteArrayInputStream(
        #""\u3053\u3093\u306b\u3061\u306f""#)
...
func inputStreamEmptyKeyed() async throws {
    let empty = try await JSON.Value.decode(from: ByteArrayInputStream("{}"))
    #expect(empty == .object([:]))
}

@Test("InputStream simple keyed")
func inputStreamSimpleKeyed() async throws {
    let stream = ByteArrayInputStream(#"{"key":"value"}"#)
...
func inputStreamNestedKeyed() async throws {
    let stream = ByteArrayInputStream(#"{"o":{"k":"v"}}"#)
...
func inputStreamWhitespaceInKeyed() async throws {
    let whitespace = try await JSON.Value.decode(from: ByteArrayInputStream(
        #"{"key" : "value"}"#))
...
func inputStreamWhitespaceBetweenKeyedValues() async throws {
    let separator = try await JSON.Value.decode(from: ByteArrayInputStream(
        #"{"k1":"v1", "k2":"v2"}"#))
...
func inputStreamUnkeyed() async throws {
    ...
    let simple = try await JSON.Value.decode(from: ByteArrayInputStream("[1,2]"))
    #expect(simple == .array([.number(.uint(1)), .number(.uint(2))]))

    let strings = try await JSON.Value.decode(from: ByteArrayInputStream(
        #"["one", "two"]"#))
...
func inputStreamNested() async throws {
    let objectInArray = try await JSON.Value.decode(from: ByteArrayInputStream(
        #"["one", 2, {"key": false}]"#))
    ...
    let arrayInObject = try await JSON.Value.decode(from: ByteArrayInputStream(
        #"{"values" : [1,true]}"#))
